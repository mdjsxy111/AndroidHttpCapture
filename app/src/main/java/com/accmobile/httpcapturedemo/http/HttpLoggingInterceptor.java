...
import com.accmobile.httpcapturedemo.util.IOUtils;
        ...
        try {
            ...
            if (isOpenCapture) {
                ...
                if ("POST".equals(method)) {
                    debugHost = APIS.baseUrl;
                    debugUrl = url.replace(debugHost, "");
                    requestBodyToString = requestString;
                } else {
                    debugHost = APIS.baseUrl;
                    debugUrl = url.replace(debugHost, "");
                    requestBodyToString = "未配置params";
                }
                HttpCaptureData.getSingleton().create()
                    .setCaptureHost(debugHost)
                    .setCaptureStatus(String.valueOf(code))
                    .setCaptureDate(dateStr)
                    .setCaptureTime(tookMs + "ms")
                    .setCaptureUrl(debugUrl)
                    .setCaptureRequestStr(requestBodyToString)
                    .setCaptureResponseStr(body)
                    .add();
            }
        }
        return response;
